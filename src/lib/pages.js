...
import { QUERY_ALL_PAGES, QUERY_PAGE_BY_URI, QUERY_PAGE_SEO_BY_URI,QUERY_ALL_PAGES_SLUG } from 'data/pages';
...
export async function getStaticPageSlug() {
  const apolloClient = getApolloClient();

  const data = await apolloClient.query({
    query: QUERY_ALL_PAGES_SLUG,
  });
  const pages = data?.data.pages.edges.map(({ node = {} }) => node).map(mapPageData).map((data)=>{
    return data.uri
  });
